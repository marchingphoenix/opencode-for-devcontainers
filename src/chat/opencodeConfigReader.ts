...
export function readOpenCodeConfig(
  ...
  // Try opencode.jsonc first, then opencode.json (matches OpenCode's own precedence).
...
/**
 * Load all agents from the OpenCode configuration for a given workspace.
 *
 ...
 */
export function loadAgentsFromOpenCodeConfig(workspaceRoot: string): {
  agents: AgentConfig[];
  ...
  // 3. Read markdown-based agents (override JSON definitions with same id).
